...
def _dct_2d(img, cutoff=100):
    ...
    return dct(
        dct(img.astype(np.float64).T, norm="ortho", n=cutoff).T, norm="ortho", n=cutoff
    )
...
def fit_gaussian_1d(f_arr, x_arr):
    ...
    """
    assert (
        len(f_arr.shape) == len(x_arr.shape) == 1
    ), f"Arrays must be 1d, got {f_arr.shape}, {x_arr.shape} instead"
    assert (
        len(f_arr) >= 3
    ), f"At least 3 points are needed for gaussian fit, received {len(f_arr)}."
    ...
    initial_guess = (
        ...
        amp_guess,
        amp_guess / 10,
    )  # Parameters: xpos, sigma, amp, offset
    try:
        popt, pcov = scipy.optimize.curve_fit(
            ...
            bounds=(
                (
                    x_arr.min(),  # min position of the peak
...
def fit_parabola_1d(f_arr, x_arr):
    ...
    assert (
        len(f_arr) >= 3
    ), f"At least 3 points are needed for fit, received {len(f_arr)}."
    x_peak = x_arr[np.argmax(f_arr)]  # initial guess for peak position
    amp_guess, offset_guess = (
        f_arr.max() / ((x_arr.max() - x_arr.min()) / 2) ** 2,
        f_arr.max(),
    )
